...
class ImageAttachmentMigrator:
    def __init__(
        ...
        self.moved_sources = set()
        self.source_to_dest = {}
        self.empty_placeholders_removed = 0
        self.unreferenced_images = []
        self.empty_dirs_pending = []

    def normalize_source(self, raw_path: str) -> str:
        path = raw_path.strip()
    ...
    def destination_for(self, source_path: Path) -> Path:
        ...
        while candidate.exists() and candidate.resolve() != source_path.resolve():
            candidate = self.attachments_dir / f"{parent_name}__{source_path.stem}_{counter}{source_path.suffix}"
            counter += 1
        return candidate
    ...
    def ensure_file_moved(self, source_path: Path) -> str | None:
        ...
        relative = dest_path.relative_to(self.vault_dir).as_posix()
        self.source_to_dest[source_key] = relative

        if dest_path.exists():
            self.moved_sources.add(source_path)
            return relative
    ...
    def replace_markdown_images(self, text: str, md_path: Path):
        changed = False
    ...
    def replace_html_images(self, text: str, md_path: Path):
        changed = False
    ...
    def replace_wikilinks(self, text: str, md_path: Path):
        changed = False
    ...
    def replace_markdown_links(self, text: str, md_path: Path):
        changed = False
    ...
    def backup_markdown(self, md_path: Path):
        backup_path = Path(str(md_path) + ".imgattachbak")
    ...
    def cleanup_empty_files_dirs(self):
        for files_dir in sorted(self.vault_dir.rglob("*_files"), reverse=True):
            if not files_dir.is_dir():
                continue
            remaining = list(files_dir.iterdir())
            if remaining:
                continue
            self.empty_dirs_pending.append(str(files_dir))
            if not self.dry_run:
                files_dir.rmdir()
            self.deleted_dirs += 1
    ...
    def find_unreferenced_images(self):
        ...
        for files_dir in sorted(self.vault_dir.rglob("*_files")):
            if not files_dir.is_dir():
                continue
            image_files = [p for p in files_dir.iterdir() if p.is_file() and p.suffix.lower() in IMAGE_EXTENSIONS]
            other_files = [p for p in files_dir.iterdir() if p.is_file() and p.suffix.lower() not in IMAGE_EXTENSIONS]

            unreferenced.extend(str(p) for p in image_files)

            would_be_empty = bool(image_files) and not other_files

            if image_files and self.delete_unreferenced_images and not self.dry_run:
                for image_path in image_files:
                    image_path.unlink()

            remaining_after = []
            if files_dir.exists():
                remaining_after = list(files_dir.iterdir())
            if not remaining_after:
                empty_dirs.append(str(files_dir))
                if self.delete_unreferenced_images and not self.dry_run:
                    files_dir.rmdir()
                    self.deleted_dirs += 1
            elif would_be_empty:
                empty_dirs.append(str(files_dir))
                if self.delete_unreferenced_images and not self.dry_run and not list(files_dir.iterdir()):
                    files_dir.rmdir()
                    self.deleted_dirs += 1

        self.unreferenced_images = unreferenced
        return unreferenced, empty_dirs

    def run(self):
        md_files = sorted(self.vault_dir.rglob("*.md"))
        for md_path in md_files:
            self.process_markdown(md_path)

        self.cleanup_empty_files_dirs()
        unreferenced_images, empty_dirs_after = self.find_unreferenced_images()
        self.empty_dirs_pending.extend([d for d in empty_dirs_after if d not in self.empty_dirs_pending])
        ...
        return {
            ...
            "empty_dirs_pending": len(self.empty_dirs_pending),
        }


def main():
    parser = argparse.ArgumentParser(
        description="Move note-local images into /attachments and rewrite Markdown to Obsidian embeds"
    )
    parser.add_argument(
        "vault",
        nargs="?",
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Remove empty image placeholders like ![]() from markdown",
    )
    parser.add_argument(
        "--delete-unreferenced-images",
        action="store_true",
        help="Delete leftover images still sitting in *_files after migration and remove empty dirs",
    )
    ...
    migrator = ImageAttachmentMigrator(
        vault_dir=vault_dir,
    ...
    print(f"Missing image sources: {result['missing_sources']}")
    ...
    print(f"Empty _files dirs ready to delete: {result['empty_dirs_pending']}")

    if migrator.skipped_missing:
        print("\nMissing sources:")
        for item in migrator.skipped_missing[:50]:
            print(item)
        if len(migrator.skipped_missing) > 50:
            print(f"... and {len(migrator.skipped_missing) - 50} more")

    if migrator.unreferenced_images:
        print("\nUnreferenced images:")
        for item in migrator.unreferenced_images[:200]:
            print(item)
        if len(migrator.unreferenced_images) > 200:
            print(f"... and {len(migrator.unreferenced_images) - 200} more")
